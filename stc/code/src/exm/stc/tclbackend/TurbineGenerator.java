...
public class TurbineGenerator implements CompilerBackend {
  ...
  public void initialize(CodeGenOptions options, ForeignFunctions foreignFuncs) {
    ...
    tree.add(new Command("set STC_VERSION ", Settings.get(Settings.STC_VERSION)));
    tree.add(new Command("set TURBINE_VERSION " , this.turbineVersion));
    tree.add(new Command("set SWIFT_FILE_PATH ", input_file.getAbsolutePath() ));
    tree.add(new Command("set LOG_FILENAME ", output_file.getAbsolutePath()));
  ...
  private void turbineStartup() {
    ...
    tree.add(new Command("set master_ptr [prov_init provenance_master.db]"));
    tree.add(new Command("puts woopwoopwoop"));
    //tree.add(new Command("check_master $master_ptr"));
    //tree.add(new Command("turbine::prov_insert_once ScriptRun scriptFileName,logFileName,swiftVersion,cogVersion,starttime $SWIFT_FILE_PATH,$LOG_FILENAME,$STC_VERSION,$TURBINE_VERSION,0 $master_ptr"));
