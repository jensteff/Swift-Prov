# Location: ${SwiftHome}/turbine/code/lib/turbine.tcl
# This is my modified version of turbine.tcl. The code does not belong to me, except for my modifications, and it is being used for research
# purposes only.
...
namespace eval turbine {
    ...
    proc get_db_ptr { rank } {
        ...
        if {[file size $db_name] == 0} {
            ...
            puts [prov_build_from_schema $ptr]
    ...
    proc finalize { } {
        ...
        if { [ info exists ::TURBINE_ADLB_COMM ] } {
            ...
            puts [prov_finalize $ptr]
            puts "\nfinalized $ptr"
            if {$rank == 0} {
                prov_finalize $prov::master
                set turbine_end_time [clock milliseconds]
                puts [expr $turbine_end_time - $turbine_start_time]
